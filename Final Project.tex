...
$$
where $D_{X}$ and $D_{Y}$ are the euclidian diameters of $X$ and $Y$. The \text{dual} problem is
$$
...
% An approximation yields \emph{noise} in the value of all the functions and \emph{thus} yields noise in the approximation of the gradients -- so the stochastic methods may be applicable.
...
\While{not done}{
  $\begin{pmatrix} x_{t+1} \\ y_{t+1} \end{pmatrix} = \pi_{X \times Y}[\begin{pmatrix} x_{t} \\ y_{t} \end{pmatrix} - \gamma_{t}\psi(x_{t},y_{t};\omega)]$\;
}
return $\begin{pmatrix} x^{t} \\ y^{t} \end{pmatrix} = \dfrac{1}{t - \tau_{*}(t) + 1}\sum_{\tau = \tau_{*}(t)}^{t}\begin{pmatrix} x_{\tau} \\ y_{\tau} \end{pmatrix}$\;
\end{algorithm}
\smallskip

[THOMAS _ WHAT IS TAU???]
...
If assumptions \ref{primal_ass1},\ref{primal_ass2},\ref{primal_ass3}, and \ref{primal_ass4} hold and if we choose the step size in the SASP to be $\gamma_{t} = C_{t}t^{-\alpha}$, $\rho_{X} = D_{X}^{2}$, $\rho_{Y} = D_{Y}^{2}$ and $\tau^{*}(t) = \dfrac{t}{2}$, then
\begin{align*}
...
$\bar{x_1} = x_1$ \;
\While{not done}{
  $y_{t+1} = \arg\min_{y \in Y} (-K\overline{x}_{t},y) + J(y) + \dfrac{1}{2\eta_{t}} \lVert y - y_{t} \rVert_{2}^{2}$\;
  $x_{t+1} = \arg\min_{x \in X} G(x) + (Kx,y_{t+1}) + \dfrac{1}{2\eta_{t}}\lVert x - x_{t} \rVert_{2}^{2} $ \;
  $\bar{x}_{t+1} = \theta_{t}(x_{t+1} - x_{t}) + x_{t+1}$ \;
}
